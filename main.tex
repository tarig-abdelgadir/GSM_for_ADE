...
%     \item $I=\{0,\ldots,N\}$
%     \item $\VW$ to be the direct sum of $\rho_i$'s that is isomorphic to defining representation
%     \item $X:=\Spec(S(\VW^\vee)^G) = \VW \GIT G$ singular affine quotient
%     \item $\bX:=\VW/G$ smooth stack quotient
...
% irreducible representations with $\rho_0$ being the trivial representation.
% Set $I=\{0,\ldots,N\}$.
% We take $\VW$ to be the direct sum of representations $\rho_i$ that is isomorphic to defining representation $\bC^2$.
% We will write $X:=\Spec(S(\VW^\vee)^G) = \VW \GIT G$ and $\bX:=\VW/G$ for the stack.
...
% We abuse notation and write $\VW$ for the direct sum of $V_i$ corresponding to the $G$-representation $\VW$.
...
% We let $L_i:= \wedge^{d_i} V_i$ and similarly $L_W:= \wedge^2 \VW$.
...
%The following lemma follows from unwinding the definitions.
%\begin{lemma}\label{lem:Btilde}
%For $(\bfx, \balpha, \bfB) \in \bA$ and an arrow $i\toj$ in $Q$, the linear map $V_i \rightarrow V_j$ in the representation $\widetilde{\bfB}(\bfx) \in \cS$ is 
%$$V_i \xrightarrow{\textup{id} \otimes \bfx} V_i \otimes \VW \xrightarrow{B_i \bigoplus_{ \rightarrow j} V_j \rightarrow V_{h(a)}.$$
%\end{lemma}
